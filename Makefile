...
all: test megacheck vendor fmt vet lint swagger | $(BASE) ; $(info $(M) building executable… ) @ ## Build program binary
	$Q cd $(BASE)/api && $(GO) build -i \
		...
		-o $(BASE)/bin/$(PACKAGE)
	$Q cd $(BASE) && cp -fp conf/logrotate bin/$(PACKAGE)_logrotate
	$Q cd $(BASE) && cp -fp conf/rsyslog.conf bin/$(PACKAGE)_rsyslog.conf
	$Q cd $(BASE) && cp -fp conf/service.template bin/$(PACKAGE).service.template
